...
posts = [
 {
  "date": "2025-11-20",
  "time": "17:49", # Note: Current time is 17:47, so this post is now in the past.
  "content": "🔥 WEDNESDAY NIGHT FOOTBALL ACTION — LET’S CASH IN! 🔥\n\nTwo massive games TONIGHT — perfect time to smash big bets and boost your winnings!\n\n⚽ Palmeiras vs Vitória\n⚽ Fluminense vs Flamengo (Huge Derby!)\n\nThese matches are LOADED with value. Don’t wait. Jump straight into Football Section and place your winning combo now!\n\n👉 Football Section:\nhttps://refpa3665.com/L?tag=d_4681277m_2170c_&site=4681277&ad=2170&r=line/football\n\n👉 FortunoBet:\nhttps://fortunobet.com\n\n💥 Don’t miss your chance — odds are FIRE tonight! Place your bets and LET’S WIN BIG! 💰🔥",
  "images": ["/home/www/bots/bots/fortunobet/second.png"]
},
{
...
def send_post(post):
    try:
        # Check if the post has images and if the image files exist
        if "images" in post and post["images"]:
            media_group = []
            valid_images = [img for img in post["images"] if os.path.exists(img)]
            
            if len(valid_images) > 1:
                # Send as album
                for idx, img_file in enumerate(valid_images):
                    # Attach the caption only to the first image
                    caption = post["content"] if idx == 0 else None
                    with open(img_file, "rb") as f:
                        media_group.append(InputMediaPhoto(f.read(), caption=caption))
                if media_group:
                    bot.send_media_group(chat_id=CHANNEL_ID, media=media_group)
                ...
                # No valid images found, send text only
                bot.send_message(chat_id=CHANNEL_ID, text=post["content"])
        else:
            # Text only
            bot.send_message(chat_id=CHANNEL_ID, text=post["content"])

        print(f"[{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}] ✅ Posted: {post['date']} at {post['time']}")
    except Exception as e:
        print(f"[{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}] ❌ Failed to post: {e}")
...
for post in posts:
    ...
    try:
        ...
        post_time = TIMEZONE.localize(run_datetime_naive) 
    except ValueError as e:
        print(f"Skipping post due to invalid date/time format: {run_date_str}. Error: {e}")
        continue
    ...
    if now >= post_time and now <= post_time + grace_period:
        print(f"[{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}] Post was missed (within 5 min grace period). Posting immediately: {post['date']} at {post['time']}")
        send_post(post)
...
# 🛑 CRITICAL FIX for duplicates: Remove all existing jobs before adding new ones
...
for post in posts:
    ...
    # 🔑 FIX for date scheduling: Use the 'date' trigger for one-time specific dates.
    # We do NOT pass a separate 'timezone' argument to avoid the dependency conflict error.
