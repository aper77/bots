...
for _k, _v in {
    "total_depositors": 0, "daily_first_five": {}, "daily_new_players": {},
    "daily_purchases": {}, "daily_depositors": {}, "daily_stars_spent": {},
    "daily_rounds_played": {}, "daily_revenue_spins": {},
    "biggest_win_ever": 0, "highest_multiplier": 0.0,
}.items():
    if _k not in stats:
        stats[_k] = _v
...
def get_user(uid: int) -> dict:
    ...
    if k not in user_data:
        ...
        save_all()
    u = user_data[k]
    if "ever_deposited" not in u:
        u["ever_deposited"] = u.get("purchases", 0) > 0
    if "total_stars_spent" not in u:
        u["total_stars_spent"] = 0
    return u
...
def check_early_bird(uid: int) -> bool:
    ...
    winners = stats["daily_first_five"].setdefault(today, [])
    k = str(uid)
    if k in winners or len(winners) >= DAILY_EARLY_LIMIT:
        return False
    winners.append(k)
    get_user(uid)["spins"] += DAILY_EARLY_SPINS
    save_all()
    return True


def is_withdraw_unlocked(u: dict) -> bool:
    stars_ok = u.get("total_stars_spent", 0) >= MIN_STARS_TO_UNLOCK
    refs_ok  = u.get("referrals", 0) >= MIN_REFS_TO_UNLOCK
    return stars_ok or refs_ok
...
def pay_link(addr: str, amt: float, comment: str) -> str:
...
def danger_bar(mult: float, crash: float) -> str:
    ...
    if crash <= 1.0:
        ...
        pct = max(0.0, min((mult - 1.0) / (crash - 1.0), 1.0))
    red   = int(round(pct * BAR))
    green = BAR - red
    bar   = "🟩" * green + "🟥" * red
    if pct < 0.3:
        label = "SAFE ZONE ✅"
            ...
            if ref_id != uid and not u.get("referred_by"):
                ...
                try:
                    await bot.send_message(
                        ref_id,
                        f"🎉 *+5 spins!* Your friend joined Fortuno Aviator!\n"
                        f"💰 Balance: *{ru['spins']}* spins\n"
                        f"👥 Referrals: *{ru['referrals']}*/{MIN_REFS_TO_UNLOCK}",
                        parse_mode="Markdown"
                    )
                except Exception:
                    pass
...
async def howtoplay(callback: types.CallbackQuery):
    await callback.message.answer(
        ...
        "✅ Bet 10 spins → cashout at 3x → WIN 30 spins!\n"
        "💥 Bar full red = plane crashes = you lose!\n\n"
        "📈 97% RTP\n"
        "💸 Min withdraw: 200 spins = $2.00\n"
        "🔓 Unlock withdraw: spend 50 Stars OR invite 5 friends",
        parse_mode="Markdown",
        reply_markup=InlineKeyboardMarkup(inline_keyboard=[
            [InlineKeyboardButton(text="✈️ Play Now!", callback_data="play")],
            [InlineKeyboardButton(text="🔙 Back",      callback_data="back_main")],
...
async def process_play(callback: types.CallbackQuery):
    ...
    if u.get("banned"):
        ...
        return
    if uid in active_games:
        await callback.answer("Game already running!", show_alert=True)
        return
    track_daily(uid)
    await callback.message.answer(
        f"✈️ *Choose Your Bet*\n"
        f"━━━━━━━━━━━━━━━━\n"
        f"💰 Balance: *{u['spins']}* spins\n"
        f"Current bet: *{u.get('bet_size', 1)}* spins\n\n"
        f"👇 Pick bet then press 🚀 FLY NOW!",
        parse_mode="Markdown",
        reply_markup=bet_kb(uid)
    )
    await callback.answer()
...
async def set_bet(callback: types.CallbackQuery):
    ...
    save_all()
    await callback.message.edit_reply_markup(reply_markup=bet_kb(uid))
    await callback.answer(f"Bet set to {bet} spins!")
...
async def start_game(callback: types.CallbackQuery):
    ...
    bet = u.get("bet_size", 1)
    if u["spins"] < bet:
        await callback.answer(f"Not enough spins! Need {bet}, have {u['spins']}.", show_alert=True)
        return
    ...
    cm = await callback.message.answer(
        f"✈️ *Plane is taking off!*\n\n"
        f"🎯 Bet: *{bet}* spins\n"
        f"⏳ *3...*\n\n"
        f"Get your finger ready on CASH OUT! 👇",
...
async def run_game(uid: int):
    try:
        ...
        logging.error(f"Game error uid={uid}: {e}")
        active_games.pop(uid, None)
...
async def my_stats(callback: types.CallbackQuery):
    uid     = callback.from_user.id
    u       = get_user(uid)
    stars   = u.get("total_stars_spent", 0)
    refs    = u.get("referrals", 0)
    unlocked = is_withdraw_unlocked(u)

    if unlocked:
        lock_line = "✅ Withdrawal UNLOCKED!"
    else:
        need_stars = MIN_STARS_TO_UNLOCK - stars
        need_refs  = MIN_REFS_TO_UNLOCK - refs
        lock_line  = f"🔒 Locked — need {need_stars} more Stars OR {need_refs} more friends"
    ...
    await callback.message.answer(
        ...
        f"🏆 Best win: *{u.get('biggest_win', 0)}* spins\n"
        f"📈 Highest mult: *{u.get('highest_multiplier', 0.0):.2f}x*\n"
        f"⭐ Stars spent: *{stars}*/{MIN_STARS_TO_UNLOCK}\n"
        f"👥 Friends invited: *{refs}*/{MIN_REFS_TO_UNLOCK}\n"
        f"📅 Member since: {u.get('joined_date', 'N/A')}\n\n"
        f"{lock_line}",
        parse_mode="Markdown",
        reply_markup=InlineKeyboardMarkup(inline_keyboard=[
...
async def referral_menu(callback: types.CallbackQuery):
    ...
    refs = u.get("referrals", 0)
    link = f"https://t.me/{BOT_NAME}?start={uid}"
    need = max(0, MIN_REFS_TO_UNLOCK - refs)

    if refs >= MIN_REFS_TO_UNLOCK:
        status = "✅ Withdrawal unlocked via referrals!"
    else:
        status = f"Invite *{need}* more friends to unlock withdrawal!"

    await callback.message.answer(
        f"👥 *Invite Friends — Earn Free Spins!*\n"
        f"━━━━━━━━━━━━━━━━━━━━━\n"
        f"For every friend you invite → *+5 free spins!*\n\n"
        f"🔗 Your link:\n`{link}`\n\n"
        f"👫 Friends invited: *{refs}*/{MIN_REFS_TO_UNLOCK}\n"
        f"🎁 Spins earned: *{refs * 5}*\n\n"
        f"{status}",
        parse_mode="Markdown",
        reply_markup=InlineKeyboardMarkup(inline_keyboard=[
            [InlineKeyboardButton(text="🔙 Back", callback_data="back_main")]
...
async def withdraw_menu(callback: types.CallbackQuery):
    ...
    if not is_withdraw_unlocked(u):
        ...
        refs       = u.get("referrals", 0)
        need_stars = MIN_STARS_TO_UNLOCK - stars
        need_refs  = max(0, MIN_REFS_TO_UNLOCK - refs)
        await callback.message.answer(
            f"🔒 *Withdrawal Locked*\n"
            f"━━━━━━━━━━━━━━━━━━━━━\n\n"
            f"To unlock choose ONE option:\n\n"
            f"💎 *Option 1 — Deposit:*\n"
            f"   Spend *{MIN_STARS_TO_UNLOCK} Stars* ($0.50)\n"
            f"   You spent: *{stars}*/{MIN_STARS_TO_UNLOCK} Stars\n"
            f"   You need *{need_stars}* more Stars\n\n"
            f"👥 *Option 2 — Invite Friends:*\n"
            f"   Invite *{MIN_REFS_TO_UNLOCK} friends*\n"
            f"   You invited: *{refs}*/{MIN_REFS_TO_UNLOCK} friends\n"
            f"   You need *{need_refs}* more friends\n\n"
            f"Complete one option to unlock! 🔓",
            parse_mode="Markdown",
            reply_markup=InlineKeyboardMarkup(inline_keyboard=[
    ...
    if u["pending_withdrawal"]:
        await callback.message.answer(
            f"⏳ *Withdrawal Pending*\n\n"
            f"You already requested *{u['pending_spins']}* spins.\n"
            f"Admin will pay within 24 hours.",
            parse_mode="Markdown",
            reply_markup=InlineKeyboardMarkup(inline_keyboard=[
                [InlineKeyboardButton(text="🔙 Back", callback_data="back_main")]
            ])
...
async def admin_dashboard(message: Message):
    ...
    pend_txt     = ""
    for uid, ud in pending[:5]:
        wallet_short = ud["pending_wallet"][:12] if ud.get("pending_wallet") else "N/A"
        pend_txt += f"  - `{uid}` — {ud['pending_spins']} spins ({usd(ud['pending_spins'])}) -> `{wallet_short}...`\n"
    if not pend_txt:
        pend_txt = "  None\n"
    ...
    await message.answer(
        ...
        f"📅 Today ({today})\n"
        f"  Active: {dau} · New: {new_p}\n"
        f"  Rounds: {rounds_today} · Purchases: {purch_today}\n"
        f"  Stars: {stars_today} · Depositors: {dep_today}\n\n"
        f"📊 All Time\n"
        f"  Players: {stats['total_players']}\n"
        f"  Rounds: {stats.get('total_rounds_played', 0)}\n"
        f"  Cashouts: {stats.get('total_cashouts', 0)} · Crashes: {stats.get('total_crashes', 0)}\n"
        f"  Biggest win: {stats.get('biggest_win_ever', 0)} spins\n"
        f"  Highest mult: {stats.get('highest_multiplier', 0.0):.2f}x\n"
...
async def stats7(message: Message):
    ...
    for i in range(7):
        ...
        new_p = stats["daily_new_players"].get(d, 0)
        rnds  = stats["daily_rounds_played"].get(d, 0)
        st    = stats["daily_stars_spent"].get(d, 0)
        lines.append(f"  `{d}` — Active:{dau} · New:{new_p} · Rounds:{rnds} · Stars:{st}")
    await message.answer("\n".join(lines), parse_mode="Markdown")
...
async def admin_userinfo(message: Message):
    ...
    try:
        ...
        if not u:
            ...
            return
        unlocked   = is_withdraw_unlocked(u)
        unlock_txt = "YES" if unlocked else "LOCKED"
        await message.answer(
            f"👤 *USER INFO* · `{target_id}`\n"
            f"━━━━━━━━━━━━━━━━━━━━\n"
            f"💰 *{u.get('spins', 0)}* spins · {usd(u.get('spins', 0))} · {ton(u.get('spins', 0))} TON\n"
            ...
            f"🏆 Best win: *{u.get('biggest_win', 0)}* spins\n"
            f"📈 Highest mult: *{u.get('highest_multiplier', 0.0):.2f}x*\n"
            f"⭐ Stars spent: *{u.get('total_stars_spent', 0)}*/{MIN_STARS_TO_UNLOCK}\n"
            f"👥 Referrals: *{u.get('referrals', 0)}*/{MIN_REFS_TO_UNLOCK}\n"
            f"🔓 Withdraw: *{unlock_txt}*\n"
            ...
            reply_markup=InlineKeyboardMarkup(inline_keyboard=[
                [InlineKeyboardButton(text="🎁 Gift spins", callback_data=f"gift_menu_{target_id}")],
                [InlineKeyboardButton(text="Ban/Unban user", callback_data=f"toggle_ban_{target_id}")],
            ])
        )
    except (IndexError, ValueError):
        await message.answer("Usage: /userinfo [user_id]")
...
async def process_buy(callback: types.CallbackQuery):
    await callback.message.answer(
        f"💎 *Buy Spins*\n\n"
        f"Minimum: *10 Stars* = 100 spins\n"
        f"Spend *{MIN_STARS_TO_UNLOCK} Stars* to unlock withdrawal!\n\n"
        f"Pick your package:",
        parse_mode="Markdown",
        reply_markup=buy_kb()
    )
...
async def process_buypack(callback: types.CallbackQuery):
    ...
    await bot.send_invoice(
        callback.message.chat.id,
        title=f"{label} — {spins} Spins",
        description=f"{spins} spins · 97% RTP · TON payouts",
        payload=f"buy_{stars}_{spins}",
        provider_token="",
        currency="XTR",
        prices=[LabeledPrice(label=f"{spins} Spins", amount=stars)],
...
async def success_pay(m: Message):
    ...
    prev_stars    = u["total_stars_spent"] - stars
    just_unlocked = u["total_stars_spent"] >= MIN_STARS_TO_UNLOCK and prev_stars < MIN_STARS_TO_UNLOCK
    unlock_msg    = "\n\n🔓 *Withdrawal UNLOCKED!* You can now withdraw!" if just_unlocked else ""

    await m.answer(
        f"✅ *+{spins} Spins added!*\n\n"
        f"💰 Balance: *{u['spins']}* spins\n"
        f"⭐ Stars spent: *{u['total_stars_spent']}*/{MIN_STARS_TO_UNLOCK}"
        f"{unlock_msg}\n\n"
        f"✈️ Fly high and win big!",
        parse_mode="Markdown",
        reply_markup=main_kb(uid)
...
async def main():
    ...
    logging.info("Fortuno Aviator is online!")
    await dp.start_polling(bot)
